...
module.exports = {
    ...
    options: [
        {
            name: 'song',
            description: 'a música que você quer pesquisar',
    ...
    async execute({ client, inter }) {
        ...
        .setDescription(`${maxTracks.map((track, i) => `**${i + 1}**. ${track.title} | ${track.author}`).join('\n')}\n\nSelecione a opção entre **1** e **${maxTracks.length}** ou **cancelar** ⬇️`)
        .setTimestamp()
        ...
        collector.on('collect', async (query) => {
            ...
            if (!value || value <= 0 || value > maxTracks.length) return inter.followUp({ content: `Resposta inválida, tente um valor entre **1** e **${maxTracks.length}** ou **cancelar**... tentar novamente? ❌`, ephemeral: true });
            ...
            try {
                ...
                return inter.followUp({ content: `Não consigo entrar no canal de voz ${inter.member}... tentar novamente? ❌`, ephemeral: true });
            }
        ...
        collector.on('end', (msg, reason) => {
            if (reason === 'time') return inter.followUp({ content:`tempo de pesquisa esgotado ${inter.member}... tentar novamente? ❌`, ephemeral: true })
        });
    },
};
